...
recipe_generator = LlmAgent(
    ...
    **TASK:**
    ...
    - If the list of ingredients are in Brazilian Portuguese translate to English.
...
image_agent = LlmAgent(
    name="image_fetcher",
    model=config.worker_model,
    instruction=f"Call the `get_ingredient_image` tool with the received ingredient name",
    tools=[get_ingredient_image],
    output_key="fetched_image",
)
...
final_recipe_presenter_agent = LlmAgent(
    ...
    tools=[get_ingredient_image],
    description="Formats the final recipe into a user-friendly markdown report with ingredient images.",
    instruction="""
    You are a food blogger creating beautiful recipe cards for parents.
    Your task is to take the final recipe data and format it into a clear and appealing markdown report.

    **CRITICAL INSTRUCTION: IMAGE INTEGRATION**
    For EVERY ingredient in the recipe's ingredient list, you MUST call the `get_ingredient_image` tool to fetch its image.
    You will then display the image next to the ingredient's name by embedding the returned base64 data using the markdown format: ![<ingredient_name>](data:image/png;base64,<image_b64>)

    **REPORT STRUCTURE:**
    -   Start with the recipe `title` as a main heading (`#`).
    -   Include the `description` and `age_appropriateness`.
    -   Use a sub-heading (`##`) for "Ingredients". For each item, list the ingredient and its corresponding image.
    -   Use a sub-heading (`##`) for "Instructions" and list the steps.
    -   Add a "Nutrition Notes" section (`##`) with a helpful summary of the health benefits.
    -   Add a "Safety First!" section (`##`) with a reminder about appropriate food texture.

    Your final output must be a single, well-formatted markdown document with all ingredient images included.
    """,
